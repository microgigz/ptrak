...
RecentActivityPanel=(function($){
    function RecentActivityPanel(){
        this.initialize();
        var self=this;
        jQuery('.secondary_details a').live('click',function(event){
            jQuery(event.target).parents('li').toggleClass('unexpanded expanded');
        });
        jQuery('#activity_feed .nav_button').click(function(event){
            jQuery(event.target).parents('.controls').find('a').toggleClass('active');
            self._playing=!self._playing;
        });
    }
    $.extend(RecentActivityPanel,{
        batchInterval:60*1000,
        serverInterval:120*1000,
        panelSize:11,
        setup:function(initialActivity){
            var panel=new RecentActivityPanel();
            panel.firstBatch(initialActivity);
            return panel;
        },
...
$.extend(RecentActivityPanel.prototype,{
    initialize:function(){
        this._data=[];
        this._updateUrl="/public_projects/recent_activity";
        this._newestCommandId=0;
        this._scrollInterval=3*1000;
        this._playing=true;
    },
    firstBatch:function(data){
        var self=this;
        self.setNewestCommandIdFromData(data);
        self._data=data.reverse();
        var numToAdd=Math.min(RecentActivityPanel.panelSize,self._data.length);
        for(var i=0;i<numToAdd;i++){
            $('#activity_feed .list').prepend(self.buildPanelItem(self._data.pop()));
        }
        self.recalculateScrollInterval();
        setTimeout(function(){
            self.nextItem();
        },this._scrollInterval);
        setTimeout(function(){
            self.nextBatch();
        },RecentActivityPanel.batchInterval);
    },
    nextItem:function(){
        var self=this;
        setTimeout(function(){
        ...
        if(this._data.length>0){
            var element=this.buildPanelItem(this._data.pop())[0];
            $('#activity_feed .list ').prepend(element);
            element.style.display="none";
            jQuery(element).slideDown('slow');
            $('#activity_feed .list > li').last().remove();
        }
    },
setNewestCommandIdFromData:function(data){
    if(data.length>0){
        this._newestCommandId=data[data.length-1].command.id;
    }
},
nextBatch:function(){
    var self=this;
    ...
    jQuery.get(self._updateUrl,{
        last_command_id:this._newestCommandId
        },function(data){
        $.each(data,function(index,item){
            self._data.unshift(item);
        });
        self.setNewestCommandIdFromData(data);
        self.recalculateScrollInterval();
    });
},
recalculateScrollInterval:function recalculateScrollInterval(){
    if(this._data.length>0){
        this._scrollInterval=parseInt(RecentActivityPanel.serverInterval/this._data.length);
    }else{
        this._scrollInterval=1000;
    }
},
buildPanelItem:function(activityData){
    var project=activityData.project,story=activityData.story,command=activityData.command;
    var projectHref="/projects/"+project.id;
    var storyType=story.story_type;
    var stories=RecentActivityPanel.numWithCommas(project.story_count);
    var sanitized_project_name=Element.sanitizeText(project.name);
    var sanitized_project_desc=Element.sanitizeText(project.description);
    var ellipsified_project_name=sanitized_project_name.ellipsify(40);
    var sanitized_story_name=Element.sanitizeText(story.name);
    var sanitized_command_description=Element.sanitizeText(command.description);
    return($('<li class="unexpanded">').append($("<div class='main_details'>").append("<h3><a class='short' href='"+projectHref+"'>"+ellipsified_project_name+"</a></h3>","<h3><a class='long'  href='"+projectHref+"'>"+sanitized_project_name+"</a></h3>","<span class='activity_description short'>"+sanitized_command_description.ellipsify(96-ellipsified_project_name.length)+"</span>","<span class='activity_description long'>"+sanitized_command_description+"</span>","<p class='short'>"+sanitized_story_name.ellipsify(53)+"</p>","<p class='long'>"+sanitized_story_name+"</p>"),$("<div class='secondary_details'>").append("<a href='#'>Details</a>","<img src='/images/v7/application/stories_view/icons/"+
        storyType+".png' alt='"+storyType+"'/>"),$("<div class='project_info long'>").append("<p class='project_description'><span>Project description:</span> "+
        sanitized_project_desc+"</p>",$("<ul class='stats'>").append("<li class='stories' title='"+stories+" stories'>"+stories,"<li class='members' title='"+project.member_count+" members'>"+project.member_count,"<li class='activity' title='"+project.command_count+" actions/week'>"+project.command_count))));
}
});
...
Element.newLink=function(content,options){
    return element=Element.newTag("a",content,options);
};
...
Element.newLinkToFunction=function(onclickFunction,options,innerText){
    ...
    YAHOO.util.Event.addListener(renderedElement,'click',onclickFunction);
    return renderedElement;
};
...
Element.newButton=function(buttonText,onclickFunction,options){
    ...
    YAHOO.util.Event.addListener(inputElement,'click',onclickFunction);
    return inputElement;
};

Element.newForm=function(onSubmitFunction){
    var submitCallback=onSubmitFunction||function(){
        return false;
    };

    var element=Element.create("form");
    element.action='#';
    YAHOO.util.Event.addListener(element,'submit',submitCallback);
    return element;
};
...
Element.linkify=function(text){
    text=text.replace(/\w+:\/\/\S+\b/g,function(url){
        var matches;
        if(matches=url.match(new RegExp(App.SHOW_STORY_URL+"/(\\d+)$"))){
            return"<a href='#' onclick='app.showStoryById("+matches[1]+")'>"+url+"</a>";
        }else{
...
Element.clear=function(element){
    j(element).html('').removeData().unbind();
};
...
Element.create=function(tagName,attributes,events){
    ...
    if(events){
        for(var eventName in events){
            YAHOO.util.Event.addListener(e,eventName,events[eventName]);
        }
        }
return e;
};
...
Element.collectAndSortAttributes=function(element){
    ...
    if(element.attributes){
        for(var i=0;i<element.attributes.length;i++){
            attributes.push(element.attributes.item(i));
        }
...
(function($){
    $.fn.obliterate=function(){
        this.removeData().html('').unbind().remove();
    }
})(jQuery);